...
public class AuthController {
    ...
    public ResponseEntity<Boolean> updateEmailOrUsername(@RequestHeader(value = "Authorization")String token,@RequestBody UpdateEmailOrUsernameRequestDto dto){ // token'ı, Header'daki 'Authorization' value'suna karşılık gelen key'den al! diyoruz.
    ...
    public ResponseEntity<List<Long>> findByRole(@RequestHeader(value = "Authorization")String token, @RequestParam String role){ // token'ı, Header'daki 'Authorization' value'suna karşılık gelen key'den al! diyoruz. ||| 'RequestHeader' ile ilgili: biz FeignClient'la yaptığımız işlemlerde, başka servisin Controller'ındaki bir metoda istek attığımızda, eğer metot kapalıysa (yetkilendirme gerektiriyorsa), 'RequestHeader' ile token'ı oraya gönderiyoruz.
